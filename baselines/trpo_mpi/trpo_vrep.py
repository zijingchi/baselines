...
def traj_segment_generator(pi, env, horizon, stochastic):
    ...
    #ac = np.concatenate((ac, np.zeros(1)))
    new = True
    rew = 0.0
    true_rew = 0.0
    ob = env.reset()
    ...
    '''obs_config = np.array([ob['joint'] for _ in range(horizon)])
    goal = np.array(ob['target'] for _ in range(horizon))
    obs_pos = np.array([ob['observation_pos'] for _ in range(horizon)])
    obs_ori = np.array([ob['observation_ori'] for _ in range(horizon)])'''
...
def load_model(path):
    saver = tf.train.Saver()
    saver.restore(U.get_session(), path)
