...
class UR5VrepEnv(UR5VrepEnvBase):
    ...
    def step(self, ac):
        ...
        done = self._angle_dis(cfg, self.target_joint_pos, self.dof) < 0.1 or self.collision_check or invalid
    ...
    def _tip_dis(self, cfg):
        return np.linalg.norm(tipcoor(cfg)[-3:]-self.tip_pos)
    ...
    def reset_obstacle(self):
        ...
        obs_pos += np.concatenate((0.08*np.random.randn(2), np.array([0.2*np.random.rand()+0.05])))
