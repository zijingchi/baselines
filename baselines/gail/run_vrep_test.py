...
def argsparser():
    ...
    parser.add_argument('--num_timesteps', help='number of timesteps per episode', type=int, default=6e5)
    # Behavior Cloning
...
def runner(env, policy_func, load_model_path, timesteps_per_batch, number_trajs,
    ...
    for _ in tqdm(range(number_trajs)):
        ...
        if suc:
            suc_count += 1
        obs, goal, obstacle, acs, ep_len, ep_ret = traj['ob'], traj['goal'], traj['obstacle_pos'], traj['ac'], traj['ep_len'], traj['ep_ret']
        obs_list.append(obs)
        acs_list.append(acs)
    ...
    if save:
        ...
        np.savez(filename, obs=np.array(obs_list), acs=np.array(acs_list),
                 lens=np.array(len_list), rets=np.array(ret_list), goal=goal_list, obstacle=obstacle_list)
...
def traj_1_generator(pi, env, horizon, stochastic):
    ...
    news = np.array(news)
    acs = np.array(acs)
    traj = {"ob": obs, "rew": rews, "new": news, "ac": acs, "goal": goal_pos,
            "obstacle_pos": obstacle_pos, "ep_ret": cur_ep_ret, "ep_len": cur_ep_len}
    return traj, suc
