...
def learn(env, policy_func, dataset, optim_batch_size=256, max_iters=1e4,
          adam_epsilon=1e-5, optim_stepsize=2e-4,
    ...
    for iter_so_far in tqdm(range(int(max_iters))):
        ...
        train_loss, g = lossandgrad(ob_expert[:, :dof], ob_expert[:, dof:2*dof], ob_expert[:, -6:-3], ob_expert[:, -3:], ac_expert, True)
        adam.update(g, optim_stepsize)
        if verbose and iter_so_far % val_per_iter == 0:
            ob_expert, ac_expert = dataset.get_next_batch(-1, 'val')
            val_loss, _ = lossandgrad(ob_expert[:, :dof], ob_expert[:, dof:2*dof], ob_expert[:, -6:-3], ob_expert[:, -3:], ac_expert, True)
